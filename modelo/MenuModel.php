		...
		Public function registrar()
		{
			...
			foreach ($_POST as $key => $value) 
			{
				...
				if($key == 'posicion_men')
				{
					...
					$menu = $_POST["menu_men"];
					$consulta = $this->Modelo->consultarSQL('menus', "status_men=1 AND id_Menu=$menu", 'assoc');
					
					if($_POST['posicion_men'] == 'ANTES')
					{
						$posicion_men = $consulta['posicion_men'];
						$postInsert['posicion_men'] = $consulta['posicion_men'];
						$consultaMenor = $this->Modelo->consultarSQL('menus', "posicion_men >= $posicion_men ", 4, 'id_Menu, posicion_men');
						//echo $consultaMenor->fetch().num_rows;
						
						if($consultaMenor->rowCount() != 0)
						{
							while($row = $consultaMenor->fetch()){
								$postEditar[$row['id_Menu']] = $row['posicion_men']+1;
							}
					...
					}elseif($_POST['posicion_men'] == 'DESPUES')
					{
						$posicion_men = $consulta['posicion_men'];
						//echo 'id='.$posicion_men;
						$postInsert['posicion_men'] = $consulta['posicion_men'] + 1;
						$consultaMayor = $this->Modelo->consultarSQL('menus', "posicion_men > $posicion_men ", 4, 'id_Menu, posicion_men');
						//echo $consultaMayor->rowCount(); //$consultaMayor->num_rows;
						//$fetch_assoc = $consultaMayor->fetch();
						//echo $consultaMayor;
						if($consultaMayor->rowCount() != 0)
						{
				...
				}elseif(($key != 'posicion_men') and ($key != 'menu_men'))
				{
				...
				if($i==sizeof($_POST)) 
				{
					...
					$postInsert['fecha_reg_men'] = $Funcion->fecha();
					$postInsert['hora_reg_men'] =  $Funcion->hora();
					$postInsert['fecha_mod_men'] = $Funcion->fecha();
					$postInsert['hora_mod_men'] =  $Funcion->hora();
					$postInsert['status_men'] = 1;
			...
			$id = $this->Modelo->insertarSQL_2('menus', $postInsert);
			//echo $id;
			if(!empty($id)){
				return true;
			}else

jQuery(document).ready(function($) {
    ...
    function getAllRoutesForMap() {
		...
		showPermanentMessage('Por favor seleccione una ruta.');
    ...
    function loadRoutes(json) {        
        if (json.length == 0) {
			...
			showPermanentMessage('No hay rutas Disponible.');
        }
        else {
    ...
    function loadGPSLocations(json) {
        ...
        if (json.length == 0) {
            ...
            showPermanentMessage('No hay datos de seguimiento para ver');
            map.innerHTML = '';
        }
        else {
    ...
    function deleteRoute() {
        if (hasMap()) {
    		...
    		confirm("Clic aquí en la página web de prueba, esto funciona muy bien.");
    		return false;
		
            //var answer = confirm("This will permanently delete this route\n from the database. Do you want to delete?");
            var answer = confirm("Esto eliminará permanentemente esta ruta \ n de la base de datos. ¿Quieres eliminar?");
            if (answer){
            ...
            alert("Por favor, seleccione una ruta antes de tratar de eliminar.");
        }
    }
